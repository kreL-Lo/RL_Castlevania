...
from posible_action import define_action_dict, action_to_array
...
import gym
import retro
from baselines import WarpFrame, FrameStack
...
def get_movie_array(basePath):
    onlyfiles = [f for f in listdir(
        basePath) if isfile(join(basePath, f))]
    return onlyfiles


def fill_expert_buffer(env, exp_buffer, movies, movie_path, nsteps=10):
    for movie in movies:
        exp_buffer = parse_demo(
            env, exp_buffer, movie_path + '\\' + movie, nsteps)
    return exp_buffer
...
def game_get_dict_key(keys):
    # b, down, left, right
    key = (keys[0], keys[4], keys[5], keys[6], keys[7], keys[8])
    return key
...
def make_env(game='Castlevania-Nes', state='Level1', stack=True, scale_rew=True, scenario='contest',
             is_contest_env=False, action_list=[['B'], ['UP'], ['DOWN'], ['LEFT'], ['RIGHT'], ['A']]):
    print('len action list', len(action_list), action_list)
...
def build_model(action_len, img_rows=84, img_cols=84, img_channels=4, dueling=True, clip_value=1.0,
    ...
    input_img = Input(shape=(img_rows, img_cols, img_channels),
                      name='input_img', dtype='float32')
    # scales the image. input is in ints of 0 to 255
    ...
    if not dueling:
        ...
        cnn_output = Lambda(dueling_operator, name='cnn_output')(
            [dueling_values, dueling_actions])
        # alternate way: https://github.com/keras-rl/keras-rl/blob/master/rl/agents/dqn.py
    cnn_model = Model(input_img, cnn_output)

    input_img_dq = Input(shape=(img_rows, img_cols, img_channels),
                         name='input_img_dq', dtype='float32')
    input_img_nstep = Input(shape=(
        img_rows, img_cols, img_channels), name='input_img_nstep', dtype='float32')
    ...
    input_expert_action = Input(
        shape=(2,), name='input_expert_action', dtype='int32')
    input_expert_margin = Input(
        shape=(action_len,), name='input_expert_margin')
...
def train_network(env, train_model, target_model, exp_buffer, rep_buffer, action_len, max_timesteps=1000000, min_buffer_size=50000,
    ...
    for i in range(850):
        env.step([0, 0, 0, 0, 0, 0, 0, 1, 0])
        env.render()

    curr_obs, _, _, _ = env.step([0, 0, 0, 0, 0, 0, 0, 1, 0])
    ...
    while train_ts < max_timesteps:
        ...
        if random.random() <= epsilon:
            action_value = random.randint(0, action_len-1)
            action_command = action_to_array(action_value)
        else:
            temp_curr_obs = np.array(curr_obs)
            temp_curr_obs = temp_curr_obs.reshape(
                1, temp_curr_obs.shape[0], temp_curr_obs.shape[1], temp_curr_obs.shape[2])
            q, _, _ = train_model.predict(
                [temp_curr_obs, temp_curr_obs, empty_by_one, empty_exp_action_by_one, empty_action_len_by_one])
            action_value = np.argmax(q)
            action_command = action_to_array(action_value)
        ...
        if train_ts % 1000 == 0:
            print("Training step %d. Epsilon: %f" % (train_ts, epsilon))
        # do action
        ...
        #total_rew += _rew
        nstep_state_deque.append(curr_obs)
        nstep_action_deque.append(action_value)
        nstep_rew_list.append(_rew)
        nstep_nexts_deque.append(_obs)
        nstep_done_deque.append(_done)
        ...
        if train_ts > min_buffer_size:
            ...
            exp_states_batch, exp_action_batch, exp_reward_batch, exp_next_states_batch, \
                exp_done_batch, exp_nstep_rew_batch, exp_nstep_next_batch = map(
                    np.array, zip(*exp_zip_batch))
            ...
            expert_margin_array = np.ones(
                (batch_size, action_len)) * expert_margin
            expert_margin_array[np.arange(
                exp_batch_size), exp_action_batch] = 0.  # expert chosen actions don't have margin
            ...
            states_batch, action_batch, reward_batch, next_states_batch, done_batch, \
                nstep_rew_batch, nstep_next_batch = map(
                    np.array, zip(*zip_batch))

            # concatenating expert and generated replays
            concat_states = np.concatenate(
                (exp_states_batch, states_batch), axis=0)
            ...
            concat_reward = np.concatenate(
                (exp_reward_batch, reward_batch), axis=0)
            concat_done = np.concatenate((exp_done_batch, done_batch), axis=0)
            concat_action = np.concatenate(
                (exp_action_batch, action_batch), axis=0)
        ...
        if train_ts % update_every == 0 and train_ts >= min_buffer_size:
            print("Saving model weights at DQfD timestep {}. Loss is {}".format(
                train_ts, loss))
            loss = np.zeros((4,))
            zString = "./dqfd_training_weights/model_{}_{}.h5".format(
                time_int, train_ts)
    ...
    zString = "dqfd_training_weights/final_model_{}_{}.h5".format(
        time_int, train_ts)
    train_model.save_weights(zString, overwrite=True)
...
def inner_train_function(train_model, target_model, exp_buffer, rep_buffer,
    ...
    dq_loss = np.square(dq_pred[np.arange(
        batch_size), action_batch]-dq_targets[np.arange(batch_size), action_batch])
    nstep_loss = np.square(nstep_pred[np.arange(
        batch_size), action_batch] - nstep_targets[np.arange(batch_size), action_batch])
    ...
    dq_loss_weighted = np.reshape(
        dq_loss, (batch_size, 1))/np.sum(dq_loss) * loss[1] * batch_size
    nstep_loss_weighted = np.reshape(
        nstep_loss, (batch_size, 1))/np.sum(nstep_loss) * loss[2] * batch_size
    ...
    if rep_buffer is not None:
        exp_buffer.update_weights(
            exp_minibatch, sample_losses[:exp_batch_size])
        rep_buffer.update_weights(
            minibatch, sample_losses[-(batch_size-exp_batch_size):])
    else:
...
def train_expert_model(train_model, target_model, exp_buffer, action_len, batch_size=32, train_steps=750000, update_every=10000,
    ...
    for current_step in range(train_steps):
        ...
        exp_margin[np.arange(
            batch_size), exp_action_batch] = 0.  # expert chosen actions don't have margin
        ...
        if current_step % update_every == 0 and current_step >= update_every:
            print("Saving expert training weights at step {}. Loss is {}".format(
                current_step, loss))
            loss = np.zeros((4,))
            zString = "./dqfd_training_weights/expert_model_{}_{}.h5".format(
                time_int, current_step)
            train_model.save_weights(zString, overwrite=True)
            # updating fixed Q network weights
            target_model.load_weights(zString)

    print('Saving expert final weights. Loss is {}'.format(loss))
    zString = "./dqfd_training_weights/expert_model_final_{}_{}.h5".format(
        time_int, train_steps)
    train_model.save_weights(zString, overwrite=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--game', help='game name',
                        default='Castlevania-Nes')
    parser.add_argument('--state', help='state', default='GreenHillZone.Act1')
    parser.add_argument('--demodir', help='demo directory',
                        default='dqfd_demos/')
    parser.add_argument(
        '--demonumber', help='number or demo files to parse', default=10)
    parser.add_argument('--train_expert', help='train expert only', default=1)
    parser.add_argument('--model_weights', help='weights to load',
                        default='./dqfd_training_weights/model_1610836184_240000.h5')
    parser.add_argument(
        '--ebuff_save_name', help='file name for saving expert buffer', default='expert_transitions')
    parser.add_argument(
        '--ebuff_load_name', help='file name for loading expert buffer', default='expert_transitions')
    ...
    if bool(False):
        env, n_action = make_env(game=args.game, stack=True, scale_rew=False,
        ...
        expert_buffer = replay.PrioritizedReplayBuffer(
            500000, alpha=0.4, beta=0.6, epsilon=0.001)

        movie_path = '.\HumanLearning\human\Castlevania-Nes\contest'
        movies = get_movie_array(movie_path)
        expert_buffer = fill_expert_buffer(
            env, expert_buffer, movies, movie_path)
        ...
        replay_buffer = replay.PrioritizedReplayBuffer(
            500000, alpha=0.4, beta=0.6, epsilon=0.001)
        # load trained model
        ...
        train_network(env, init_model, tgt_model, expert_buffer, replay_buffer,
                      n_action, max_timesteps=250000, epsilon_start=0.2, min_buffer_size=1000)  # ,min_buffer_size=50
