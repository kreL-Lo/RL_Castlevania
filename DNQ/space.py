...
while episode <= EPISODES:
    ...
    current_state, stacked_frames = stack_frames(
        stacked_frames, current_state, True)
    start = time.time()
    done = False
    ...
    while step < MAXSTEPTS:
        ...
        if done:
            ...
            new_state, stacked_frames = stack_frames(
                stacked_frames, new_state, False)
            step = MAXSTEPTS
        ...
        for x in recorder:
            sum1 += x
        d = [round(recorder[0]/sum1, 2), round(recorder[1]/sum1, 2),
             round(recorder[2]/sum1, 2)]
        if step % 1000 == 1:
            print(stats, rew, action, qs, episode, d,
                  EPSILON, step, qs1, np.argmax(qs1))
        qs = 0
    print(EPSILON)
    ...
    stringul = str(episode) + " , " + str(des1)+" , " + \
        str(episode_reward) + " , " + str(stats) + " , " + str(d) + "\n"
    f.write(stringul)
